...
/*
*********************************************************************************************************
*	函 数 名: mpu_dmp_get_pitch
*	功能说明: 根据DMP处理后的数据计算俯仰角pitch
*	形    参: *pitch：俯仰角
*	返 回 值: 0：正常
 *	         1：异常
*	注    意：该函数完全照抄上方正点原子编写的mpu_dmp_get_data，唯一的区别在于该函数只获取了欧拉角中的俯仰角；该函数的优
 *	         点在于只计算获取了一个角度，针对性强
*********************************************************************************************************
*/
u8 mpu_dmp_get_pitch(float *Pitch,short *Gyro)
{
//    float pitch=0;  //俯仰角
    float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;
    unsigned long sensor_timestamp;
    ...
    long quat[4];
    if(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors,&more))return 1;
    *Gyro=gyro[1];  //这里需要修改一下把gyro提出来，用于平衡小车，这里gyro共有三个，取中间值提出，与俯仰角对应
    /* Gyro and accel data are written to the FIFO by the DMP in chip frame and hardware units.
     * This behavior is convenient because it keeps the gyro and accel outputs of dmp_read_fifo and mpu_read_fifo consistent.
    **/
    /*if (sensors & INV_XYZ_GYRO )
